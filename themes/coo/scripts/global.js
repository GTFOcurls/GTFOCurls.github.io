...
hexo.extend.helper.register("icon", function (slug,img) {
    //console.log(`### Looking for ${slug} icon ${img}`);
...
hexo.extend.generator.register('json', function (locals) {
    ...
    datas.each(function (data) {
        ...
        {
            //console.log("### JSON icon: "+data.icon);
            temp_data.icon = svgIcon(data.icon);
        }
        else
        {
            //console.log("### JSON slug: "+data.slug);
...
hexo.extend.generator.register('list', function (locals) {
    ...
    themeConfig.index_categories.forEach(function (category) {
        ...
        locals.categories.findOne({name: category}).posts.sort('-date').map(function (post) {
            content += `- [${post.title}](https://gtfocurls.github.io/${post.path}): ${post.intro.trim()}\n`;
        })
        content += `\n</details>\n\n`;
    });
...
function svgIcon(name) {
    ...
    if (fs.existsSync(svgPath)) {
        ...
        //console.log("### Not Found svgPath:"+svgPath)
        return "<svg viewBox=\"0 0 1024 1024\" xmlns=\"http://www.w3.org/2000/svg\" fill=\"currentColor\" height=\"1em\" width=\"1em\">\n" +
